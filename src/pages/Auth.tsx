...
const Auth: React.FC = () => {
  ...
  const handleSubmit = async (e: React.ChangeEvent<HTMLFormElement>) => {
    e.preventDefault();
    setIsLoading(true);
    setToastMessage(isSignUp ? 'Creating your account...' : 'Signing in...');
    setShowToast(true);

    try {
      let response;
      if (isSignUp) {
        response = await api.auth.signup(formData.name, formData.email, formData.password);
      } else {
        response = await api.auth.signin(formData.email, formData.password);
      }

      if (response.data?.token) {
        setToastMessage(isSignUp 
      ...
      if (error.response?.status === 409 || 
        ...
        setToastMessage('User already exists! Redirecting to sign in...');
        setShowToast(true);
        setTimeout(() => {
          setIsSignUp(false);
        }, 2000);
          ...
          {isSignUp && (
            <div>
              ...
              <input
                ...
                maxLength={30}
                pattern="[A-Za-z]+"
                title="Name must contain only letters and be between 3-30 characters"
